...
# Validate web panel port even when interactive = no
# Asking for a panel port
if [ -z "$control" ]; then
	while validate_control; do
		echo -e "\nPlease use a valid port for web panel (ex. 8083)."
		read -p 'Please enter valid web panel port: ' control
	done
else
	if validate_control; then
		echo "Please use a valid port for web panel (ex. 8083)."
		exit 1
	fi
fi
...
write_config_value "BACKEND_PORT" "$control"
